        ...
        v-layout(wrap='')
          ...
          v-flex(xs12='')
            v-toolbar(color='cyan', dark='')
              v-toolbar-title.usuarios_Style Publicaciones
              v-spacer
              v-text-field(v-model='search', append-icon='search', label='Buscar por Título', single-line='', hide-details='', color='white', style='color:white;', @keyup="filterPosts")
          v-flex(xs12='')
            ...
            v-card
              v-container(fluid='', grid-list-md='', style='padding: 0px;')
                v-layout(wrap='')
                  v-flex(xs12, v-for='card in cardsFilter', v-bind='{ [`xs${card.flex}`]: true }', :key='card.photour')
                    <!--h1 Hola-->
                    v-card
    ...
    methods: {
      ...
      filterPosts() {
        ...
        this.cardsFilter = this.cards.filter(function (lf) {
          return lf.data.body.toUpperCase().includes(self.search.toUpperCase())
        })
      },
      listPost() {
